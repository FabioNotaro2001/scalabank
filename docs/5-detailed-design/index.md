...
Per modellare il concetto di banca è stato speso diverso tempo per analizzare se fosse meglio inserire il riferimento della banca alle varie altre entità (cliente e impiegato) o il contrario, ovvero inserire i riferimenti degli impiegati e dei clienti all'interno della banca.
...
- un conto corrente può avere (uso degli `Options`) un saving jar, il quale è un'astrazione per una sorta di salvadanaio che annualmente produce degli interessi.
...
- La classe `CurrencyConverter` è l'implementazione del trait sopra descritto &rarr; si noti che essa contiene tra le altre cose anche un campo privato `ExchangeRateProvider` che permetterà in qualche modo la conversione di valuta
