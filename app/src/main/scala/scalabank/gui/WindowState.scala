...
object GUI:
  ...
  def run(): Unit =
    val loginView = for
      ...
      _ <- addLabel("CF-Label", "Codice fiscale:", "CF-Label-Panel", FlowLayout.CENTER)
        ...
        e.map:
            ...
            for
              ...
              _ <-
                ...
                customer match
                  case Some(c) =>
                    for
                      _ <- changeLabel("User-Title", s"Benvenuto ${c.name} ${c.surname}")
          ...
          case "User-Home-Account" =>
            for
              ...
              _ <- changeLabel("User-Account-Title", s"Conto ${accId}")
              _ <- exec:
                account = customer.get.bankAccounts.find(_.id.toString == accId)
              _ <- changeLabel("Account-Amount", s"Bilancio: ${account.get.balance.toString} ${account.get.currency.symbol}")
              _ <- updateList(
                "Op-List",
