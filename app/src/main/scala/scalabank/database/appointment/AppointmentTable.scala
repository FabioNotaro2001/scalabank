...
/**
 * Trait for the appointment Table.
 */
trait AppointmentTable extends AbstractCache[Appointment, (String, String, String)] with DatabaseOperations[Appointment, (String, String, LocalDateTime)]:
  /**
...
/**
 * Companion object for AppointmentTable
 */
object AppointmentTable:

  /**
   * Creates a new AppointmentTable instance.
   *
   * @param connection The database connection to use.
   * @param database   The database reference.
   * @return A new AppointmentTable instance.
   */
  def apply(connection: Connection, database: Database): AppointmentTable = AppointmentTableImpl(connection, database)

  private class AppointmentTableImpl(override val connection: Connection, override val database: Database) extends AppointmentTable:
    import database.*
    ...
    override def findByEmployeeCf(employeeCf: String): Seq[Appointment] =
      val stmt = connection.prepareStatement("SELECT * FROM appointment WHERE employeeCf = ?")
      findByCf(stmt, employeeCf)

    override def findByCustomerCf(customerCf: String): Seq[Appointment] =
