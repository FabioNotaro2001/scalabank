...
import scalabank.database.{AbstractCache, Database, DatabaseOperations, HasCache}
...
/**
 * Trait representing the operations for the bank account table.
 */
trait BankAccountTable extends AbstractCache[BankAccount, Int] with DatabaseOperations[BankAccount, Int]:

  /**
   * Find bank accounts by customer fiscal code.
   *
   * @param customerCf The fiscal code of the customer.
   * @return A sequence of bank accounts associated with the given customer fiscal code.
   */
  def findByCustomerCf(customerCf: String): Seq[BankAccount]


/**
 * Companion object for creating instances of BankAccountTable.
 */
object BankAccountTable:

  /**
   * Creates a new instance of BankAccountTable.
   *
   * @param connection The database connection to use.
   * @param database   The database reference.
   * @return A new instance of BankAccountTable.
   */
  def apply(connection: Connection, database: Database): BankAccountTable = BankAccountTableImpl(connection, database)

  private class BankAccountTableImpl(override val connection: Connection, override val database: Database)
    extends BankAccountTable:
    ...
    override def findByCustomerCf(customerCf: String): Seq[BankAccount] =
